...
class StreamingDataProcessor:
    ...
    def download_and_process_c4(self, max_samples: int = 500000) -> Iterator[Dict]:
        """Download and process C4 dataset from Hugging Face in 5% chunks."""
        logger.info("Downloading C4 dataset in 5% chunks...")
        from datasets import load_dataset
        import os

        os.makedirs("data/pretraining/c4", exist_ok=True)
        total_count = 0
        ...
        for i in range(0, 100, 5):
            ...
            out_path = f"data/pretraining/c4/c4_{start:02d}_{end:02d}.jsonl"
            if not os.path.exists(out_path):
                logger.info(f"Downloading C4 split: {split}")
                try:
                    ...
                    logger.error(f"Failed to download C4 split {split}: {e}")
                    continue
            else:
                logger.info(f"Found existing file {out_path}, skipping download.")

            # Now process the local file
            with open(out_path, 'r', encoding='utf-8') as f:
                for line in tqdm(f, desc=f"Processing C4 {start}-{end}%"):
                    try:
                        ...
                        if len(text) > 50:
                            ...
                            if max_samples and total_count >= max_samples:
                                logger.info(f"Reached max_samples={max_samples} for C4.")
                                return
                    except json.JSONDecodeError:
                        continue

        logger.info(f"✅ Processed {total_count} samples from C4 dataset.")
    ...
    def download_and_process_dialogstudio(self, max_samples: int = 1500000) -> Iterator[Dict]:
        ...
        for subset_idx, subset in enumerate(tqdm(dialogstudio_subsets, desc="DialogStudio Sub-datasets", unit="dataset")):
            ...
            try:
                ...
                logger.info(f"Processing {subset} in streaming mode (length unknown)")
                count = 0
                for i, sample in enumerate(tqdm(dataset, desc=f"Processing {subset}", total=max_samples, leave=False)):
                    if i >= max_samples:
                        break
...
if __name__ == "__main__":
    processor = StreamingDataProcessor(tokenizer_name="gpt2", max_length=512, chunk_size=1000)
    manager = DatasetManager(base_path="data")
    manager.download_all_datasets(processor)
